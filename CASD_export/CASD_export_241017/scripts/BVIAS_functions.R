  ...
  tmp_y <- input %>%
    ...
    mutate(
      y = case_when(
        y <0 ~ 0,
        y>1 ~1,
        # I know that A.3.2 produces NAs with optimized constants instead of 1 so I set 1 for NAs
        metric_number == "A.3.2" & is.na(y) ~ 1,
        .default = y
      )
    )
...
BVIAS_MSE <- function(input,id_cols,var_constant,var_weight) {
  ...
  tmp_LU_distance <- tmp_BVIAS_median$BVIAS %>%
    ...
    select(archetype,land_use_type,litt_values,BVAS)
  ...
  tmp_qmin <- mean(c(
    tmp_BVIAS_A.5.1$BVIAS$BVAS[tmp_BVIAS_A.5.1$BVIAS$id_col == "A.5.1_q5" & tmp_BVIAS_A.5.1$BVIAS$land_use_type == "arable"],
    tmp_BVIAS_A.5.1$BVIAS$BVAS[tmp_BVIAS_A.5.1$BVIAS$id_col == "A.5.1_q5" & tmp_BVIAS_A.5.1$BVIAS$land_use_type == "grassland"]
  ))
  tmp_qmax <- tmp_BVIAS_A.5.1$BVIAS$BVAS[tmp_BVIAS_A.5.1$BVIAS$id_col == "A.5.1_q95" & tmp_BVIAS_A.5.1$BVIAS$land_use_type == "arable"]
  
  tmp_ES_A.5.1 <- (tmp_qmin - tmp_qmax) / tmp_qmin
  tmp_ES_A.5.1 <- tibble(archetype = "ES_A.5.1",land_use_type = "arable",litt_values = 0.35,BVAS = tmp_ES_A.5.1)
  ...
  tmp_qmin <- mean(c(
    tmp_BVIAS_A.4.5$BVIAS$BVAS[tmp_BVIAS_A.4.5$BVIAS$id_col == "A.4.5_q5" & tmp_BVIAS_A.4.5$BVIAS$land_use_type == "arable"],
    tmp_BVIAS_A.4.5$BVIAS$BVAS[tmp_BVIAS_A.4.5$BVIAS$id_col == "A.4.5_q5" & tmp_BVIAS_A.4.5$BVIAS$land_use_type == "grassland"]
  ))
  tmp_qmax <- mean(c(
    tmp_BVIAS_A.4.5$BVIAS$BVAS[tmp_BVIAS_A.4.5$BVIAS$id_col == "A.4.5_q95" & tmp_BVIAS_A.4.5$BVIAS$land_use_type == "arable"],
    tmp_BVIAS_A.4.5$BVIAS$BVAS[tmp_BVIAS_A.4.5$BVIAS$id_col == "A.4.5_q95" & tmp_BVIAS_A.4.5$BVIAS$land_use_type == "grassland"]
  ))
  
  tmp_ES_A.4.5 <- (tmp_qmin - tmp_qmax) / tmp_qmin
  tmp_ES_A.4.5 <- tibble(archetype = "ES_A.4.5",land_use_type = LU,litt_values = 0.35,BVAS = tmp_ES_A.4.5)
  ...
  tmp_qmin <- tmp_BVIAS_A.3.3$BVIAS$BVAS[tmp_BVIAS_A.3.3$BVIAS$id_col == "1_crop"]
  tmp_qmax <- tmp_BVIAS_A.3.3$BVIAS$BVAS[tmp_BVIAS_A.3.3$BVIAS$id_col == "3_crops"]
  tmp_ES_A.3.3 <- (tmp_qmax - tmp_qmin) / tmp_qmax
  tmp_ES_A.3.3 <- tibble(archetype = "ES_A.3.3",land_use_type = LU,litt_values = 0.18,BVAS = tmp_ES_A.3.3) %>%
  ...
  tmp_qmin <- tmp_BVIAS_A.2.1$BVIAS$BVAS[tmp_BVIAS_A.2.1$BVIAS$id_col == "A.2.1_q25"]
  tmp_qmax <- tmp_BVIAS_A.2.1$BVIAS$BVAS[tmp_BVIAS_A.2.1$BVIAS$id_col == "A.2.1_q75"]
  tmp_ES_A.2.1 <- (tmp_qmax - tmp_qmin) / tmp_qmax
  tmp_ES_A.2.1 <- tibble(archetype = "ES_A.2.1",land_use_type = LU,litt_values = 0.11,BVAS = tmp_ES_A.2.1)
  ...
  tmp_qmin <- tmp_BVIAS_A.2.2$BVIAS$BVAS[tmp_BVIAS_A.2.2$BVIAS$id_col == "A.2.2_q25"]
  tmp_qmax <- tmp_BVIAS_A.2.2$BVIAS$BVAS[tmp_BVIAS_A.2.2$BVIAS$id_col == "A.2.2_q75"]
  tmp_ES_A.2.2 <- (tmp_qmax - tmp_qmin) / tmp_qmax
  tmp_ES_A.2.2 <- tibble(archetype = "ES_A.2.2",land_use_type = LU,litt_values = -0.05,BVAS = tmp_ES_A.2.2) %>%
    filter(land_use_type == "arable")
  ...
  tmp_org_mean <- tmp_BVIAS$BVIAS %>%
    filter(crop == "111") %>% # keep only cereals
  ...
  tmp_comp_org <- (tmp_org_mean$BVAS_mean[tmp_org_mean$org_farming == T]
                   / tmp_org_mean$BVAS_mean[tmp_org_mean$org_farming == F])
  tmp_comp_org <- tibble(archetype = "comp_org",land_use_type = "arable",litt_values = 1.34,BVAS = tmp_comp_org)
  
  
  #  MSE ----
  
  distance_table <- Reduce(rbind,
                           list(tmp_LU_distance,tmp_ES_A.2.1,tmp_ES_A.2.2,tmp_ES_A.3.3,tmp_ES_A.4.5,tmp_ES_A.5.1,tmp_comp_org)) %>%
    # estimate distance
    ## we choose a fourth power distance to ensure a higher equity among distance minimization
    mutate(
      distance = (litt_values - BVAS)^4
    )
...
BVIAS_optim <- function(var_v) {
  ...
  #var_weight_v = tmp_var_weight$weight[tmp_var_weight_optim$land_use_type == tmp_LU]
  #var_constant = tmp_var_BVC_constant
  #var_constant_v = unlist(tmp_var_BVC_constant %>% filter(land_use_type == tmp_LU & metric_number %in% unique(input$metric_number)) %>% select_if(.,is.numeric))
  #var_v = c(tmp_var_constant_v_new,tmp_var_weight_v_new)
  ...
  var_constant = tmp_var_BVC_constant %>%
    ...
    cbind(.,matrix(var_constant_v,
                   nrow = nrow(tmp_var_BVC_constant %>% 
                                 filter(land_use_type == tmp_LU &
                                          metric_number %in% unique(input$metric_number))),
                   ncol = length(var_constant_v) / nrow(tmp_var_BVC_constant %>%
                                                          filter(land_use_type == tmp_LU &
                                                                   metric_number %in% unique(input$metric_number))),
                   dimnames = list(1:nrow(tmp_var_BVC_constant %>% 
                                            filter(land_use_type == tmp_LU &
                                                     metric_number %in% unique(input$metric_number))),
                                   colnames(tmp_var_BVC_constant %>% 
    ...
    mutate(
      epsilon = tmp_var_BVC_constant$epsilon[tmp_var_BVC_constant$metric_number %in% unique(input$metric_number)],
      gamma = tmp_var_BVC_constant$gamma[tmp_var_BVC_constant$metric_number %in% unique(input$metric_number)],
      beta = tmp_var_BVC_constant$beta[tmp_var_BVC_constant$metric_number %in% unique(input$metric_number)]      
    )
...
BVIAS_effect_size <- function(input,id_cols,var_constant,var_weight) {
  ...
  tmp_ES_A.2.1 <- tmp_BVIAS_A.2.1$BVIAS %>%
    pivot_wider(id_cols = c(id_col,land_use_type),
                names_from = quantile,values_from = BVIAS_ha) %>%
    mutate(BVIAS_ES_25_75 = (q75-q25)/q75, BVIAS_ES_5_95 = (q95-q5)/q95)
