...
```{r tbl_intro, echo=FALSE, include=TRUE}
...
To detail the crop routes of each production and husbandry practices for different categories of livestock, data from other studies are used as reference averages or as a distribution key (@fig-diagram; @ministere_de_lagriculture_ssp_pratiques_2019, @sailley_quantifier_2021, @jayet_european_2023, @noauthor_tables_nodate)

In order to take into account the landscape elements implemented on the farms, we use the French Land Parcel Information System (LPIS; @ign_registre_2020) and the Hedges layer of the BD TOPO® database (@ign_bd_2020) to define the average size of the plots, the hedge density (i.e., hedge linear meter per hectare) and the cultural diversity (Shannon Index). We also use ***XXX add here data from Ludovic*** to estimate the soil cover (i.e., number of day per year of covered soil).
...
### Matching the AC/FQS/FADN database with the LPIS
...
The BVIAS model estimate requires documenting the intensity of four main practices (see section ***2.1***): tillage, total nitrogen fertilization (mineral and organic), share of mineral nitrogen fertilization and use of plant protection products.
...
***check section***
In order to take into account the landscape dimension in the BVIAS model, we estimate three parameters from the pairing between the LPIS, the Hedge layer of the BD Topo® and the FADN-AC-FQS matched base: the density of hedges, the average plot size and crop diversity. Hedge density is the ratio of the sum in linear meters of hedge to the utilized agricultural area (UAA) (***See Supplementary Material***). The average size of the plots is the ratio of the UAA to the number of plots. For cultural diversity, we calculate the Shannon diversity index of agricultural cover types (@eq-3).

$$
SHDI = - \sum_{i}^{N} p_i \ln p_i
$$ {#eq-3}

where $p$ is the proportion of the area of crop cover type $i$ (including grasslands) on the farm total area of crops.
...
'\*' Initial value for 1 kg of Comté, corrected for the ratio of 10 litres of milk required to produce 1 kg of Comté (@husson_pdo_2019)
