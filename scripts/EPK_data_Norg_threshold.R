...
tmp_EPK_Norg_all <- tmp_epk_2017 %>%
  ...
  summarise(
    ...
    Norg_thrhld = N_org - Norg_CI95
    ) %>%
  # quality check : pour ici, quand Norg_thrhld < 0, je mets zero
  mutate(
    Norg_thrhld = case_when(
      Norg_thrhld < 0 ~ 0,
      .default = Norg_thrhld
    )
  ) 

write.csv(tmp_EPK_Norg_all,"~/BiodivLabel/data_out/PKGC_N_ferti_org.csv",quote = F,row.names = T)
...
tmp_EPK_Norg <- tmp_EPK_Norg_all %>%
  ...
  summarise(
    ...
    Norg_thrhld = min(Norg_thrhld,na.rm = T)) %>% ungroup()

write.csv(tmp_EPK_Norg,"~/BiodivLabel/data_out/PKGC_N_ferti_org_threshold.csv",quote = F,row.names = T)
