...
tmp_sales_milk <- tmp_sales %>%
  filter(
    ...
    farm_id %in% unique(RICA_2020$farm_id[RICA_2020$OTEFDD %in% c(4500,4700,6184)]) & # 1800 farms
...
tmp_prod <- RICA_2020_pan %>%
  # add units
  left_join(., tmp_TT_livestock_product %>% select(code_anim_prod,QPROD7_unit)) %>%
  # summarise production and convert to kg
  group_by(farm_id,code_anim_prod) %>%
  summarise(prod_kg = sum(QPROD7*QPROD7_unit)) %>% # 2379 farms
...
tmp_prod_milk <- tmp_prod %>%
  filter(
    # filter farms in OTEX diary cattle, mixed cattle, mixed farming
    farm_id %in% unique(RICA_2020$farm_id[RICA_2020$OTEFDD %in% c(4500,4700,6184)]) & # 1403 farms
      # filter by product: keep milk and full milk cow cheese (only 3 farms with mixed milk cheese => discard)
      code_anim_prod %in% c(21,22)) %>% # 1340 farms
  ...
  group_by(farm_id) %>%
  summarise(prod_kg = sum(prod_kg)) %>% ungroup()
...
tmp_BVI_milk <- BVI_to_RICA_herd %>%
  ...
  mutate(
    BVI_kg = (BVI_herd * p100_sales) / prod_kg
    )
