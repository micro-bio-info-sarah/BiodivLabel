...
tmp_VariablesRA <- RA2020 %>%
  select(NOM_DOSSIER,SIEGE_REG, REGL_1305_2013,OTEFDA_COEF17,PBSTOT_COEF17,SAU_TOT,UGBAG.TOT) %>%
...
for (tmp_product in sort(unique(tmp_nb_farm$product_name[tmp_nb_farm$nobs >=30]))) {
  ...
  for (tmp_code in unique(tmp_loop1_data$FQS[tmp_loop1_data$FQS != "Conventionnel"])){
    ...
    tmp_loop2_match_obj<-matchit(label_bool ~ SIEGE_REG + MNT + OTEFDA_COEF17 + SAU_TOT + UGBAG.TOT + AGE + NivForm,
    ...
    tmp_loop2_docx = read_docx(paste0(tmp_output_dir,"matching_summary_",Sys.Date(),".docx")) |> 
      body_add_par(paste0(tmp_product," - ",tmp_code),style = "heading 2") |>
      # covariate summary before and after matching
      body_add_par("Covariate distribution before matching",style = "heading 3") |>
      body_add_table(as.data.frame(
        datasummary_balance(data = tmp_loop2_data %>% 
                              select(product_FQS,SIEGE_REG,MNT,OTEFDA_COEF17,SAU_TOT,UGBAG.TOT,AGE,NivForm),
                            ~product_FQS,output = "data.frame"))) |>
      body_add_par("Covariate distribution after matching",style = "heading 3") |>
      body_add_table(as.data.frame(
        datasummary_balance(data = tmp_loop2_data %>% 
      ...
      body_add_par("Matching model coefficients",style = "heading 3") |>
      body_add_table(as.data.frame(tmp_loop2_model_coef)) |>
      # propensity score distribution
      body_add_par("Propensity score distribution",style = "heading 3") |>
      body_add_plot(plot(tmp_loop2_match_obj, type="jitter", interactive=FALSE)) |>
      # covariate balance
      body_add_par("Covariate balance",style = "heading 3") |>
...
tmp_nobs = read_docx(paste0(tmp_output_dir,"matching_summary_",Sys.Date(),".docx")) |> 
  body_add_par("Number of matches",style = "heading 2") |>
  body_add_table(as.data.frame(
