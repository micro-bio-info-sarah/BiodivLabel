...
BVIAS_to_RICA_RA_SIQO_crops <- BVIAS_to_RICA_crops %>%
  ...
  filter(is.finite(BVIAS_ha+BVIAS_kg))
# 5463 farms
...
BVIAS_to_RICA_RA_SIQO_milk <- BVIAS_to_RICA_milk %>%
  select(farm_id,org_farming,feed_ha_p_species,BVIAS_ha_feed,p100_sales,prod_kg,BVIAS_kg) %>%
  # add RICA_RA data
  inner_join(.,RICA_RA %>%
               rename(farm_id = IDENT),
  ...
  # remove farms with NAs
  filter(is.finite(BVIAS_ha_feed+BVIAS_kg))
 # 1302 farms
...
BVIAS_to_RICA_RA_SIQO <- bind_rows(
  ...
  select(farm_id,NOM_DOSSIER,land_use_type,production_type,crop,BVAS,BVIAS_ha,BVIAS_kg,LIBELLE_PRODUIT,SIQO_FILIERE,SIQO,org_farming,FQS)  %>%
