...
# extract descriptives of the covariates
datasummary_balance(data = tmp_basePscore[tmp_basePscore$product_RICA_CODE%in%c("111","118"),],
                    ~product_FQS,
                    output = paste0(tmp_output_dir,"tmp_desc_stat_covar_w0_match1.csv"))
datasummary_balance(data = tmp_basePscore[!(tmp_basePscore$product_RICA_CODE%in%c("111","118","milk")),],
                    ~product_FQS,
                    output = paste0(tmp_output_dir,"tmp_desc_stat_covar_w0_match2.csv"))
datasummary_balance(data = tmp_basePscore[tmp_basePscore$product_RICA_CODE%in%c("milk"),],
                    ~product_FQS,
                    output = paste0(tmp_output_dir,"tmp_desc_stat_covar_w0_match3.csv"))
...
#write.csv(tmp_estimates,paste0(tmp_output_dir,"tmp.csv",quote = F,row.names = F))

datasummary_balance(data = tmp_matched_data_all[tmp_matched_data_all$product_RICA_CODE%in%c("111","118"),],
                    ~product_FQS,
                    output = paste0(tmp_output_dir,"tmp_desc_stat_covar_match1.csv"))
datasummary_balance(data = tmp_matched_data_all[!(tmp_matched_data_all$product_RICA_CODE%in%c("111","118","milk")),],
                    ~product_FQS,
                    output = paste0(tmp_output_dir,"tmp_desc_stat_covar_match2.csv"))
datasummary_balance(data = tmp_matched_data_all[tmp_matched_data_all$product_RICA_CODE%in%c("milk"),],
                    ~product_FQS,
                    output = paste0(tmp_output_dir,"tmp_desc_stat_covar_match3.csv"))

# observe distribution des scores
#plot(tmp_matched_obj_all[[tmp_product]][[tmp_code]], type="jitter", interactive=FALSE)
# Covariate balance
#love.plot(tmp_matched_obj_all[[tmp_product]][[tmp_code]], binary="std")
...
write.csv(tmp_table,paste0(tmp_output_dir,"~/BiodivLabel/data_out/tmp_resp_var_w_match.csv"),quote = F,row.names = F)
...
for (tmp_data_in_ls in c("BVI","practice_crops","practice_herd","practice_feed")) {
  ...
  loop1_practice_names <- tmp_practice_names_all[[tmp_data_in_ls]]
  loop1_practice_data <- tmp_practice_data_all[[tmp_data_in_ls]]
...
for (tmp_data_in_ls in c("BVI","practice_crops","practice_herd","practice_feed")) {
  ...
  for (tmp_var in loop1_practice_names$practice) {
    ...
    for (tmp_match in sort(unique(tmp_matched_data_all$match[
      tmp_matched_data_all$production_type %in% unique(loop1_practice_data$production_type)]))) {

  # subset data
  loop2_data = tmp_matched_data_all %>% 
...
for (tmp_product in sort(unique(tmp_base$product_RICA_CODE[tmp_base$label != "Conventionnel"]))) {
  ...
  for (tmp_code in unique(loop1_data$label[loop1_data$label != "Conventionnel"])){ # bug avec les Fromages de Savoie
    ...
    loop2_plot_distri <- plot(loop2_match_obj, type="jitter", interactive=FALSE)# observe distribution des scores
    loop2_plot_love <- love.plot(loop2_match_obj, binary="std")# observe distribution des scores
    loop2_matched_data <-match.data(loop2_match_obj)
    ...
    tmp_plot_all[[tmp_product]][[tmp_code]][["distri"]] <- loop2_plot_distri
    tmp_plot_all[[tmp_product]][[tmp_code]][["love"]] <- loop2_plot_love
...
datasummary_skim(tmp_matched_data_all)
