...
```{r BVI ha kg w0 matching}

# run plot ha
tmp <- tmp_plot_data
# run plot ton
tmp <- bind_rows(tmp,tmp_plot_data)

# combine and filter
tmp_plot_data <- tmp %>% 
  filter(FQS %in% c("Conventional","Organic Farming","Label Rouge",
                    "Comté - Morbier","Charentes-Poitou Butter"))
...
ggplot() +
  ...
  # n
  geom_text(data = tmp_plot_data,
            aes(x = plot_x, y=(mean)*1,colour = FQS,label = paste0("n = ",as.character(nobs))),
            size=5,position = position_dodge2(width = 1),hjust = 0,vjust=1.5) +
  # stat groups
  geom_text(data = tmp_plot_data,
            aes(x = plot_x,y=(mean-sd),colour = FQS,label = stat_grp),
            size=5,position = position_dodge(width = 1),vjust = 1.5)+
  # theme
  coord_flip() +
  #ylim(c(0,max(mean-sd)*1.1)) +
  #ylim(c(min(tmp_plot_data$mean-tmp_plot_data$sd)*0.9,max(tmp_plot_data$mean+tmp_plot_data$sd)*1.2)) +
  facet_grid(vars(production_type),vars(practice),scales = "free") +
  labs(y = "BVI",x= "Products") +
  scale_fill_manual(values = tmp_FQS_names$colors,labels = tmp_FQS_names$labels,) +
  scale_colour_manual(values = tmp_FQS_names$colors,labels = tmp_FQS_names$labels) +
  theme_light() +
  theme(legend.position = "none",
...
tmp_plot_data <- tmp_table0 %>%
  # select only one practice for some products
  filter(
    practice_subset %in% c("crops","BVIAS")
    & production_type == "crop"
    & product_name %in% tmp_product_names$labels[tmp_product_names$plot == T]
    & FQS %in% tmp_FQS_names$labels[tmp_FQS_names$plot == T]) %>%
  # add conventional mean
