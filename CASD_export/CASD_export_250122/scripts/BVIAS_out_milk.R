...
if (my_DB == "RICA") {
  ...
  tmp_sales <- bind_rows(
    ...
    mutate(
      sales_total_p_species = sum(sales)
    ) %>% ungroup()
  ...
  tmp_sales_milk <- tmp_sales %>%
    ...
    group_by(farm_id,sales_total_p_species) %>%
    summarise(sales_milk = sum(sales),
              .groups = "keep") %>%
    # calculate allocation ratio
    mutate(p100_sales = sales_milk / sales_total_p_species) %>% ungroup()
...
if (my_DB == "FADN") {
  ...
  tmp_sales <- FADN_18 %>%
    ...
    group_by(ID,species) %>%
    mutate(
      sales_total_p_species = sum(sales_euros)
    ) %>% ungroup() %>%
    # filter farms in OTEX diary cattle, mixed cattle, mixed farming
  ...
  tmp_sales_milk <- tmp_sales %>%
    ...
    mutate(p100_sales = sales_euros / sales_total_p_species) %>% ungroup()
