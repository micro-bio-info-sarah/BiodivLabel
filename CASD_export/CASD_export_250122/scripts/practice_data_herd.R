...
#"Yield (L of milk / ha pseudofarm)",
  mutate(yield_l_pha_ps = prod_kg / (area_feed_grassland+area_feed_produced+area_feed_purchased)) %>%
#"Yield (L of milk / ha farm)",
  mutate(yield_l_pha = prod_kg / (area_feed_grassland+area_feed_produced)) %>%
#"Yield (L of milk / dairy cow)",
  mutate(yield_l_panim = prod_kg / cow_pop) %>% 
#"Livestock density (dairy cow / ha pseudofarm)",
  mutate(nb_cow_pha_ps = cow_pop / (area_feed_grassland+area_feed_produced+area_feed_purchased)) %>%
#"Livestock density (dairy cow / ha farm)",
  mutate(nb_cow_pha = cow_pop / (area_feed_grassland+area_feed_produced)) %>%
#"Livestock density (dairy cow / ha MFA)",
  mutate(nb_cow_pMFA = cow_pop / (MFA_dairy_cow)) %>%
#"Main Forage Area (MFA / dairy cow)",
  mutate(MFA_pcow = MFA_dairy_cow / cow_pop) %>%
#"Share of Main Forage Area (ha MFA / ha pseudofarm)",
  mutate(MFA_pha_ps = MFA_dairy_cow / (area_feed_grassland+area_feed_produced+area_feed_purchased)) %>%
#"Share of Main Forage Area (ha MFA / ha farm)",
  mutate(MFA_pha = MFA_dairy_cow / (area_feed_grassland+area_feed_produced)) %>%
#"Share of permanent grassland (ha permanent grassland / ha pseudofarm)",
  mutate(
    ha_perm_grassland_pha_ps = area_feed_grassland / (area_feed_grassland+area_feed_produced+area_feed_purchased),
#"Share of permanent grassland (ha permanent grassland / ha farm)",
    ha_perm_grassland_pha = area_feed_grassland / (area_feed_grassland+area_feed_produced)) %>%
#"Share of temporary grassland (ha temporary grassland / ha pseudofarm)",
  mutate(ha_temp_grassland_pha_ps = area_ha_temp_pasture / (area_feed_grassland+area_feed_produced+area_feed_purchased)) %>%
#"Share of temporary grassland (ha temporary grassland / ha farm)",
  mutate(ha_temp_grassland_pha = area_ha_temp_pasture / (area_feed_grassland+area_feed_produced)) %>%
#"Share of legumes (ha legumes / ha pseudofarm)",
  mutate(protein_crop_ha_pha_ps = protein_crops_ha / (area_feed_grassland+area_feed_produced+area_feed_purchased)) %>%
#"Share of legumes (ha legumes / ha farm)",
  mutate(protein_crop_ha_pha = protein_crops_ha / (area_feed_grassland+area_feed_produced)) %>%
