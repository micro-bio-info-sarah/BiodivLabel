...
if (my_DB == "RICA") {
  ...
  tmp_input <- RICA_2020_ani %>%
    ...
    left_join(.,tmp_TT_livestock %>% select(code_livestock,EFFEC6_unit,UGB,species,IPCC_mix_cat)) %>%
    # summaries populations
    group_by(farm_id,code_livestock,species,IPCC_mix_cat,UGB) %>%
...
if (my_DB == "RICA") {
  ...
  ### select milk data
  tmp_milk <- RICA_2020_pan %>%
    filter(CODE7 %in% c(21,22,31,32)) %>%
    group_by(IDENT) %>%
    summarise(
    ...
    inner_join(.,tmp_input %>% filter(code_livestock == 929)) %>%
    # calculate MILK kg animal-1 day-1
    mutate(MILK = (MILK_total / livestock_unit/ UGB)/365)
...
tmp_cattle_Nexc %>%
  ...
  filter(code_livestock %in% c(929,"LCOWDAIR")) %>%
  mutate(quantile = c(0.1,0.25,0.5,0.75,0.9))
## mediane OK
