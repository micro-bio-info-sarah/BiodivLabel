...
if (my_DB == "RICA") {
  ...
  tmp_practices <- tmp_input %>%
    ...
    bind_rows(.,
              BVIAS_to_RICA_crops %>%
                mutate(practice = "BVI_ha",value = BVI_ha) %>%
                select(farm_id,land_use_type,crop,org_farming,practice,value) %>%
                distinct()
    ) %>%
    bind_rows(.,
              BVIAS_to_RICA_crops %>%
                mutate(practice = "BVI_kg",value = BVI_kg) %>%
                select(farm_id,land_use_type,crop,org_farming,practice,value) %>%
                distinct()
    ) %>%
    # add yields
...
tmp_pseudofarm_purchased_fr <- feed_purchased %>%
  # add production mode and filter farms for which we could not estimate a BVI (i.e., inner join with tmp_practices)
...
tmp_pseudofarm_purchased <- tmp_pseudofarm_purchased_fr %>%
  mutate(
    ...
    # BVIAS
    BVI_ha = case_when(
      crop == "223" & feed_type == "feed_concent" ~ tmp_BVIAS_soybean$BVIAS_full$BVI_ha,
      .default = BVI_ha
    ),
    BVI_kg = case_when(
      crop == "223" & feed_type == "feed_concent" ~ tmp_BVIAS_soybean$BVIAS_full$BVI_kg,
      .default = BVI_kg
    )
  ) %>% ungroup()
...
tmp_BVIAS_herd <- tmp_pseudofarm_livestock_n_feed %>%
  ...
  summarise(
    ...
    # BVI_herd: total BVIAS of the feed for the herd
